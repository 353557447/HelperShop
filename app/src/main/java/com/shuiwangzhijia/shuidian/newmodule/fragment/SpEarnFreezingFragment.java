...
import com.shuiwangzhijia.shuidian.newmodule.adapter.SpEarnFreezingAdapter;
 ...
 */
@FndViewInject(contentViewId = R.layout.fragment_sp_earn_freezing)
public class SpEarnFreezingFragment extends BaseLazyFragment implements SwipeRefreshLayout.OnRefreshListener {
    @BindView(R.id.rv)
    RecyclerView mRv;
    @BindView(R.id.swipeRefreshLayout)
    SwipeRefreshLayout mSwipeRefreshLayout;
    private SpEarnFreezingAdapter mSpEarnFreezingAdapter;
    private List<ShopEarnApplyWdBean.DataBean> mList;

    public SpEarnFreezingFragment() {
    ...
    private void initRv() {
        ...
        mSpEarnFreezingAdapter = new SpEarnFreezingAdapter(mContext);
        mList = new ArrayList<>();
        mSpEarnFreezingAdapter.setData(mList);
        mRv.setAdapter(mSpEarnFreezingAdapter);
    }
    ...
    private void getData() {
        showLoadingDialog();
        RetrofitUtils.getInstances().create().shopEarnApplyWDList(mToken, mPage, mLimit, 2).enqueue(new Callback<Object>() {
            @Override
                ...
                try {
                    ...
                    if (code == 200) {
                        ...
                        if (!isLoadMore) {
                            mList.clear();
                            if (list.size() == 0)
                                setCentreViewShow(R.drawable.no_order_data_icon, "暂无冻结中收益~");
                        }
                        mList.addAll(list);
                        mSpEarnFreezingAdapter.notifyDataSetChanged();
                    } else {
                        if (!isLoadMore)
                            setCentreViewShow(R.drawable.no_order_data_icon, "暂无冻结中收益~");
                    }
